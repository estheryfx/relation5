...
def extract_spoes(text,start_rate=0.5,end_rate=0.5):
    """抽取输入text所包含的三元组
    """
    ...
    if subjects:
        ...
        for subject, object_pred in zip(subjects, object_preds):
            start = np.where(object_pred[:, :, 0] > start_rate)
            end = np.where(object_pred[:, :, 1] > end_rate)
            for _start, predicate1 in zip(*start):
                for _end, predicate2 in zip(*end):
                    if _start <= _end and predicate1 == predicate2:
                        ...
                        break
        # if len(spoes) == 0 and start_rate>0.49:
        #     return extract_spoes(text,start_rate*0.9,end_rate*0.9)
...
def evaluate(data):
    ...
    for d in tqdm(data):
        R = set([spo for spo in extract_spoes(d['text'])])
        # print(R)
        ...
        f.write(s + '\n')
    f.close()
    f1, precision, recall = 2 * X / (Y + Z), X / max(Y,1), X / Z
    print(
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            train_model.save_weights('train/best_model.weights')
